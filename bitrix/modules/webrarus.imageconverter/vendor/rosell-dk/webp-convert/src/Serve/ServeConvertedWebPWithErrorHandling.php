...
use Rarus\Isolated\WebPConvert\Serve\ServeConvertedWeb;
...
class ServeConvertedWebPWithErrorHandling
{
    ...
    private static function processOptions($options)
    {
        ...
        $options2->addOptions(new StringOption('fail', 'original', ['original', '404', 'throw', 'report']), new StringOption('fail-when-fail-fails', 'throw', ['original', '404', 'throw', 'report']));
     ...
     * @return  void
     */
    public static function serve($source, $destination, $options = [], $serveLogger = null, $convertLogger = null, $serveClass = 'Rarus\\Isolated\\WebPConvert\\Serve\\ServeConvertedWebP')
    {
        ...
        try {
            ...
            if ($e instanceof \Rarus\Isolated\WebPConvert\Exceptions\WebPConvertException) {
                Header::addLogHeader($e->getShortMessage(), $serveLogger);
            }
            self::performFailAction($options['fail'], $options['fail-when-fail-fails'], $source, $destination, $options, $e, $serveClass);
        }
    }
}
