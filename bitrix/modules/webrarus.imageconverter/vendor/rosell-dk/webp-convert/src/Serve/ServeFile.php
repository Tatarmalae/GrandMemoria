...
class ServeFile
{
    ...
    private static function processOptions($options)
    {
        ...
        $options2->addOptions(new ArrayOption('headers', []), new StringOption('cache-control-header', 'public, max-age=31536000'));
        ...
        $headerOptions->addOptions(new BooleanOption('cache-control', \false), new BooleanOption('content-length', \true), new BooleanOption('content-type', \true), new BooleanOption('expires', \false), new BooleanOption('last-modified', \true), new BooleanOption('vary-accept', \false));
    ...
    public static function serve($filename, $contentType, $options = [])
    {
        ...
        InputValidator::checkMimeType($filename, ['image/jpeg', 'image/png', 'image/webp', 'image/gif']);
        /*
        ...
        if (!empty($options['cache-control-header'])) {
            ...
            if ($options['headers']['expires']) {
                ...
                if (\preg_match('#max-age\\s*=\\s*(\\d*)#', $options['cache-control-header'], $matches)) {
                    $seconds = $matches[1];
                    Header::setHeader('Expires: ' . \gmdate('D, d M Y H:i:s \\G\\M\\T', \time() + \intval($seconds)));
                }
            }
        }
