...
namespace Rarus\Isolated\WebPConvert\Serve;

use Rarus\Isolated\WebPConvert\Convert\Exceptions\ConversionFailedException;
use Rarus\Isolated\WebPConvert\Helpers\InputValidator;
use Rarus\Isolated\WebPConvert\Helpers\MimeType;
use Rarus\Isolated\WebPConvert\Helpers\PathChecker;
use Rarus\Isolated\WebPConvert\Serve\Exceptions\ServeFailedException;
use Rarus\Isolated\WebPConvert\Serve\Header;
use Rarus\Isolated\WebPConvert\Serve\Report;
use Rarus\Isolated\WebPConvert\Serve\ServeFile;
...
use Rarus\Isolated\WebPConvert\Options\SensitiveArrayOption;
...
class ServeConvertedWebP
{
    ...
    private static function processOptions($options)
    {
        ...
        $options2->addOptions(new BooleanOption('reconvert', \false), new BooleanOption('serve-original', \false), new BooleanOption('show-report', \false), new BooleanOption('suppress-warnings', \true), new BooleanOption('redirect-to-self-instead-of-serving', \false), new ArrayOption('serve-image', []), new SensitiveArrayOption('convert', []));
    ...
    public static function serve($source, $destination, $options = [], $serveLogger = null, $convertLogger = null)
    {
        ...
        if ($options['suppress-warnings']) {
            \set_error_handler(array('\\WebPConvert\\Serve\\ServeConvertedWebP', "warningHandler"), \E_WARNING | \E_USER_WARNING | \E_NOTICE | \E_USER_NOTICE);
        }
