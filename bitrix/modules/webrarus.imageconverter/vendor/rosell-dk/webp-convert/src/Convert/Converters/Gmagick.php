...
class Gmagick extends AbstractConverter
{
    ...
    {
        return ['near-lossless', 'size-in-percentage'];
    }
    /**
    ...
    public function checkOperationality()
    {
        ...
        if (!\class_exists('Gmagick')) {
            throw new SystemRequirementsNotMetException('Gmagick is installed, but not correctly. The class Gmagick is not available');
        }
        ...
        switch ($mimeType) {
            case 'image/png':
                if (!\in_array('PNG', $im->queryFormats())) {
                    throw new SystemRequirementsNotMetException('Gmagick has been compiled without PNG support and can therefore not convert this PNG image.');
                }
                break;
            case 'image/jpeg':
                if (!\in_array('JPEG', $im->queryFormats())) {
                    throw new SystemRequirementsNotMetException('Gmagick has been compiled without Jpeg support and can therefore not convert this Jpeg image.');
                }
                break;
        }
    }
    ...
    protected function doActualConvert()
    {
        ...
        try {
            ...
            throw new ConversionFailedException('Failed creating Gmagick object of file', 'Failed creating Gmagick object of file: "' . $this->source . '" - Gmagick threw an exception.', $e);
        }
            ...
            if (!\is_null($options['preset'])) {
                if ($options['preset'] != 'none') {
                    ...
                    switch ($imageHint) {
                        ...
                        case 'text':
                            $imageHint = 'graph';
                            $this->logLn('The "preset" value was mapped to "graph" because gmagick does not support "drawing",' . ' "icon" and "text", but grouped these into one option: "graph".');
                    }
