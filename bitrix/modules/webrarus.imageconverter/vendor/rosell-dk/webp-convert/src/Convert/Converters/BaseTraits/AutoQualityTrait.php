...
use Rarus\Isolated\WebPConvert\Convert\Helpers\JpegQualityDetector;
/**
...
trait AutoQualityTrait
{
    ...
    public abstract function getMimeTypeOfSource();
    ...
    private function processQualityOption()
    {
        ...
        */
        ...
        if ($q == 'auto') {
            ...
            $this->logLn('*Setting "quality" to "auto" is deprecated. ' . 'Instead, set "quality" to a number (0-100) and "auto-limit" to true. ');
            $this->logLn('*"quality" has been set to: ' . $options['max-quality'] . ' (took the value of "max-quality").*');
            if (!$this->options2->getOptionById('auto-limit')->isValueExplicitlySet()) {
                $options['auto-limit'] = \true;
                $this->logLn('*"auto-limit" has been set to: true."*');
            } else {
                $this->logLn('*PS: "auto-limit" is set to false, as it was set explicitly to false in the options."*');
            }
            $useDeprecatedDefaultQuality = \true;
        }
        if ($options['auto-limit']) {
            if ($this->getMimeTypeOfSource() == 'image/jpeg') {
                $this->logLn('Running auto-limit');
                $this->logLn('Quality setting: ' . $q . '. ');
                $q = JpegQualityDetector::detectQualityOfJpg($source);
                if (\is_null($q)) {
                    $this->logLn('Quality of source image could not be established (Imagick or GraphicsMagick is required). ' . 'Sorry, no auto-limit functionality for you. ');
                    if ($useDeprecatedDefaultQuality) {
                        $q = $options['default-quality'];
                        $this->logLn('Using default-quality (' . $q . ').');
                    } else {
                        $q = $options['quality'];
                        $this->logLn('Using supplied quality (' . $q . ').');
                    }
                    $this->qualityCouldNotBeDetected = \true;
                } else {
                    $this->logLn('Quality of jpeg: ' . $q . '. ');
                    if ($q < $options['quality']) {
                        $this->logLn('Auto-limit result: ' . $q . ' ' . '(limiting applied).');
                    } else {
                        $q = $options['quality'];
                        $this->logLn('Auto-limit result: ' . $q . ' ' . '(no limiting needed this time).');
                    }
                }
            ...
            $this->logLn('Quality: ' . $q . '. ');
            if ($this->getMimeTypeOfSource() == 'image/jpeg') {
                $this->logLn('Consider enabling "auto-limit" option. This will prevent unnecessary high quality');
            }
        }
