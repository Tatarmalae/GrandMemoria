...
use Rarus\Isolated\WebPConvert\Convert\Exceptions\ConversionFailed\FileSystemProblems\CreateDestinationFileException;
...
class Imagick extends AbstractConverter
{
    ...
    public function checkOperationality()
    {
        ...
        if (!\class_exists('\\Imagick')) {
            throw new SystemRequirementsNotMetException('iMagick is installed, but not correctly. The class Imagick is not available');
        }
        $im = new \Imagick();
        ...
        switch ($mimeType) {
            case 'image/png':
                if (!\in_array('PNG', $im->queryFormats('PNG'))) {
                    throw new SystemRequirementsNotMetException('Imagick has been compiled without PNG support and can therefore not convert this PNG image.');
                }
                break;
            case 'image/jpeg':
                if (!\in_array('JPEG', $im->queryFormats('JPEG'))) {
                    throw new SystemRequirementsNotMetException('Imagick has been compiled without Jpeg support and can therefore not convert this Jpeg image.');
                }
                break;
        }
    }
    ...
    protected function doActualConvert()
    {
        ...
        if (!\is_null($options['preset'])) {
            if ($options['preset'] != 'none') {
                ...
                switch ($imageHint) {
                    ...
                    case 'text':
                        $imageHint = 'graph';
                        $this->logLn('The "preset" value was mapped to "graph" because imagick does not support "drawing",' . ' "icon" and "text", but grouped these into one option: "graph".');
                }
        ...
        if ($options['sharp-yuv'] === \true) {
            if (\version_compare($versionNumber, '7.0.8-26', '>=')) {
                ...
                $this->logLn('Note: "sharp-yuv" option is not supported in your version of ImageMagick. ' . 'ImageMagic >= 7.0.8-26 is required', 'italic');
            }
        }
        if ($options['metadata'] == 'none') {
            ...
            $profiles = $im->getImageProfiles("icc", \true);
