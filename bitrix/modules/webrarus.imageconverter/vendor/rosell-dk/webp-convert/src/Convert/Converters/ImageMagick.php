...
class ImageMagick extends AbstractConverter
{
    ...
    public function checkOperationality()
    {
        ...
        if (!$this->isInstalled()) {
            throw new SystemRequirementsNotMetException('imagemagick is not installed (cannot execute: "' . $this->getPath() . '")');
        }
    ...
    private function createCommandLineOptions($versionNumber = 'unknown')
    {
        ...
        if (!\is_null($options['preset'])) {
            ...
            if ($options['preset'] != 'none') {
                ...
                switch ($imageHint) {
                    ...
                    case 'text':
                        $imageHint = 'graph';
                        $this->logLn('The "preset" value was mapped to "graph" because imagemagick does not support "drawing",' . ' "icon" and "text", but grouped these into one option: "graph".');
                }
        ...
        if ($options['near-lossless'] != 100) {
            if (\version_compare($versionNumber, '7.0.10-54', '>=')) {
                ...
                $this->logLn('Note: "near-lossless" option is not supported in your version of ImageMagick. ' . 'ImageMagic >= 7.0.10-54 is required', 'italic');
            }
        }
