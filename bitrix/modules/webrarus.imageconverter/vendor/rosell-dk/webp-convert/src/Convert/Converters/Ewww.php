...
class Ewww extends AbstractConverter
{
    ...
    {
        return OptionFactory::createOptions([['api-key', 'string', ['title' => 'Ewww API key', 'description' => 'ewww API key. ' . 'If you choose "auto", webp-convert will ' . 'convert to both lossy and lossless and pick the smallest result', 'default' => '', 'sensitive' => \true, 'ui' => ['component' => 'password']]], ['check-key-status-before-converting', 'boolean', ['title' => 'Check key status before converting', 'description' => 'If enabled, the api key will be validated (relative inexpensive) before trying ' . 'to convert. For automatic conversions, you should enable it. Otherwise you run the ' . 'risk that the same files will be uploaded to ewww cloud service over and over again, ' . 'in case the key has expired. For manually triggered conversions, you can safely disable ' . 'the option.', 'default' => \true, 'ui' => ['component' => 'checkbox']]]]);
    }
    protected function getUnsupportedDefaultOptions()
    {
        return ['alpha-quality', 'auto-filter', 'encoding', 'low-memory', 'method', 'near-lossless', 'preset', 'sharp-yuv', 'size-in-percentage'];
    }
    /**
    ...
    public function checkOperationality()
    {
        ...
        if ($apiKey === \false) {
            if (isset($this->options['key'])) {
                throw new InvalidApiKeyException('The "key" option has been renamed to "api-key" in webp-convert 2.0. ' . 'You must change the configuration accordingly.');
            }
            throw new InvalidApiKeyException('Missing API key.');
        }
        if (\strlen($apiKey) < 20) {
            throw new InvalidApiKeyException('Api key is invalid. Api keys are supposed to be 32 characters long - ' . 'the provided api key is much shorter');
        }
    ...
    */
    ...
    protected function doActualConvert()
    {
        ...
        $postData = ['api_key' => $this->getKey(), 'webp' => '1', 'file' => \curl_file_create($this->source), 'quality' => $this->getCalculatedQuality(), 'metadata' => $options['metadata'] == 'none' ? '0' : '1'];
        \curl_setopt_array($ch, [\CURLOPT_URL => "https://optimize.exactlywww.com/v2/", \CURLOPT_HTTPHEADER => ['User-Agent: WebPConvert', 'Accept: image/*'], \CURLOPT_POSTFIELDS => $postData, \CURLOPT_BINARYTRANSFER => \true, \CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \false, \CURLOPT_SSL_VERIFYPEER => \false]);
        ...
        if (\curl_getinfo($ch, \CURLINFO_CONTENT_TYPE) != 'application/octet-stream') {
            ...
            */
            ...
            throw new ConversionFailedException('ewww api did not return an image. It could be that the key is invalid. Response: ' . $response);
        }
    ...
    public static function keepSubscriptionAlive($source, $key)
    {
        ...
        \curl_setopt_array($ch, [\CURLOPT_URL => "https://optimize.exactlywww.com/v2/", \CURLOPT_HTTPHEADER => ['User-Agent: WebPConvert', 'Accept: image/*'], \CURLOPT_POSTFIELDS => ['api_key' => $key, 'webp' => '0', 'file' => \curl_file_create($source), 'domain' => $_SERVER['HTTP_HOST'], 'quality' => 60, 'metadata' => 0], \CURLOPT_BINARYTRANSFER => \true, \CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \false, \CURLOPT_SSL_VERIFYPEER => \false]);
        $response = \curl_exec($ch);
        if (\curl_errno($ch)) {
    ...
    public static function getQuota($key)
    {
        ...
        $response = \curl_exec($ch);
        return $response;
        // ie -830 23. Seems to return empty for invalid keys
        // or empty
        //echo $response;
    }
}
