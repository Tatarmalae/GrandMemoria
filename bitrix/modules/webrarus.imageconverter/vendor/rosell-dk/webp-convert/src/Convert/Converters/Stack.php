...
use Rarus\Isolated\WebPConvert\Convert\ConverterFactory;
...
class Stack extends AbstractConverter
{
    ...
    {
        return ['alpha-quality', 'auto-filter', 'encoding', 'low-memory', 'metadata', 'method', 'near-lossless', 'preset', 'sharp-yuv', 'size-in-percentage', 'skip', 'default-quality', 'quality', 'max-quality'];
    }
    public function getUniqueOptions($imageType)
    {
        return OptionFactory::createOptions([['converters', 'array', ['title' => 'Converters', 'description' => 'Converters to try, ordered by priority.', 'default' => self::getAvailableConverters(), 'sensitive' => \true, 'ui' => ['component' => 'multi-select', 'options' => self::getAvailableConverters(), 'advanced' => \true]]], ['converter-options', 'array', ['title' => 'Converter options', 'description' => 'Extra options for specific converters.', 'default' => [], 'sensitive' => \true, 'ui' => null]], ['preferred-converters', 'array', ['title' => 'Preferred converters', 'description' => 'With this option you can move specified converters to the top of the stack. ' . 'The converters are specified by id.', 'default' => [], 'ui' => null]], ['extra-converters', 'array', ['title' => 'Extra converters', 'description' => 'Add extra converters to the bottom of the stack', 'default' => [], 'sensitive' => \true, 'ui' => null]], ['shuffle', 'boolean', ['title' => 'Shuffle', 'description' => 'Shuffles the converter order on each conversion. ' . 'Can for example be used to spread out requests on multiple cloud converters', 'default' => \false, 'ui' => ['component' => 'checkbox', 'advanced' => \true]]]]);
        /*
                return [
    ...
    {
        return ['cwebp', 'vips', 'imagick', 'gmagick', 'imagemagick', 'graphicsmagick', 'wpc', 'ffmpeg', 'ewww', 'gd'];
    }
    /**
    ...
    public function checkOperationality()
    {
        if (\count($this->options['converters']) == 0) {
            throw new ConverterNotOperationalException('Converter stack is empty! - no converters to try, no conversion can be made!');
        }
    ...
    protected function doActualConvert()
    {
        ...
        $defaultConverterOptions['_skip_input_check'] = \true;
        $defaultConverterOptions['_suppress_success_message'] = \true;
        unset($defaultConverterOptions['converters']);
        ...
        foreach ($converters as $converter) {
            ...
            $beginTime = \microtime(\true);
            $this->ln();
            $this->logLn($converterId . ' converter ignited', 'bold');
            $converter = ConverterFactory::makeConverter($converterId, $this->source, $this->destination, $converterOptions, $this->logger);
            try {
                $converter->doConvert();
                ...
                $anyRuntimeErrors = \true;
            }
            $this->logLn($converterId . ' failed in ' . \round((\microtime(\true) - $beginTime) * 1000) . ' ms');
        }
        $this->ln();
        $this->logLn('Stack failed in ' . \round((\microtime(\true) - $beginTimeStack) * 1000) . ' ms');
