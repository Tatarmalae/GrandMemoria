...
use Rarus\Isolated\WebPConvert\Convert\Converters\ConverterTraits\CloudConverterTrait;
use Rarus\Isolated\WebPConvert\Convert\Converters\ConverterTraits\CurlTrait;
...
use Rarus\Isolated\WebPConvert\Convert\Exceptions\ConversionFailed\ConverterNotOperational\InvalidApiKeyException;
...
class Wpc extends AbstractConverter
{
    ...
    {
        return OptionFactory::createOptions([['api-key', 'string', ['title' => 'API key', 'description' => 'The API key is set up on the remote. Copy that.', 'default' => '', 'sensitive' => \true, 'ui' => ['component' => 'password', 'advanced' => \false, 'display' => "option('wpc-api-version') != 0"]]], ['secret', 'string', ['title' => 'Secret', 'description' => '', 'default' => '', 'sensitive' => \true, 'ui' => ['component' => 'password', 'advanced' => \false, 'display' => "option('wpc-api-version') == 0"]]], ['api-url', 'string', ['title' => 'API url', 'description' => 'The endpoint of the web service. Copy it from the remote setup', 'default' => '', 'sensitive' => \true, 'ui' => ['component' => 'password', 'advanced' => \false]]], ['api-version', 'int', ['title' => 'API version', 'description' => 'Refers to the major version of Wpc. ' . 'It is probably 2, as it is a long time since 2.0 was released', 'default' => 2, 'minimum' => 0, 'maximum' => 2, 'ui' => ['component' => 'select', 'advanced' => \false, 'options' => [0, 1, 2]]]], ['crypt-api-key-in-transfer', 'boolean', ['title' => 'Crypt API key in transfer', 'description' => 'If checked, the api key will be crypted in requests. ' . 'Crypting the api-key protects it from being stolen during transfer', 'default' => \false, 'ui' => ['component' => 'checkbox', 'advanced' => \true, 'display' => "option('wpc-api-version') >= 1"]]]]);
        /*return [
    ...
    {
        $salt = '';
        $validCharsForSalt = \array_merge(\range('A', 'Z'), \range('a', 'z'), \range('0', '9'), ['.', '/']);
        for ($i = 0; $i < 22; $i++) {
    ...
    public function checkOperationality()
    {
        ...
        if ($this->getApiUrl() == '') {
            if (isset($this->options['url']) && $this->options['url'] != '') {
                throw new ConverterNotOperationalException('The "url" option has been renamed to "api-url" in webp-convert 2.0. ' . 'You must change the configuration accordingly.');
            }
            throw new ConverterNotOperationalException('Missing URL. You must install Webp Convert Cloud Service on a server, ' . 'or the WebP Express plugin for Wordpress - and supply the url.');
        }
        if ($apiVersion == 0) {
            ...
            if ($options['crypt-api-key-in-transfer']) {
                if (!\function_exists('crypt')) {
                    throw new ConverterNotOperationalException('Configured to crypt the api-key, but crypt() function is not available.');
                }
                if (!\defined('CRYPT_BLOWFISH')) {
                    throw new ConverterNotOperationalException('Configured to crypt the api-key. ' . 'That requires Blowfish encryption, which is not available on your current setup.');
                }
            }
        }
    ...
    */
    ...
    {
        $options = $this->options;
        $postData = ['file' => \curl_file_create($this->source), 'options' => \json_encode($this->createOptionsToSend()), 'servername' => isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : ''];
        $apiVersion = $options['api-version'];
    ...
    protected function doActualConvert()
    {
        ...
        \curl_setopt_array($ch, [\CURLOPT_URL => $this->getApiUrl(), \CURLOPT_POST => 1, \CURLOPT_POSTFIELDS => $this->createPostData(), \CURLOPT_BINARYTRANSFER => \true, \CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \false, \CURLOPT_SSL_VERIFYPEER => \false]);
        ...
        if ($httpCode == 404) {
            \curl_close($ch);
            throw new ConversionFailedException('WPC was not found at the specified URL - we got a 404 response.');
        }
        // Check for empty response
        if (empty($response)) {
            throw new ConversionFailedException('Error: Unexpected result. We got nothing back. ' . 'HTTP CODE: ' . $httpCode . '. ' . 'Content type:' . \curl_getinfo($ch, \CURLINFO_CONTENT_TYPE));
        }
        ...
        if (\curl_getinfo($ch, \CURLINFO_CONTENT_TYPE) != 'application/octet-stream') {
            ...
            if (\substr($response, 0, 1) == '{') {
                $responseObj = \json_decode($response, \true);
                if (isset($responseObj['errorCode'])) {
                    switch ($responseObj['errorCode']) {
                        case 0:
                            throw new ConverterNotOperationalException('There are problems with the server setup: "' . $responseObj['errorMessage'] . '"');
                        case 1:
                            throw new InvalidApiKeyException('Access denied. ' . $responseObj['errorMessage']);
                        default:
                            throw new ConversionFailedException('Conversion failed: "' . $responseObj['errorMessage'] . '"');
                    }
                }
            }
            // WPC 0.1 returns 'failed![error messag]' when conversion fails. Handle that.
            if (\substr($response, 0, 7) == 'failed!') {
                throw new ConversionFailedException('WPC failed converting image: "' . \substr($response, 7) . '"');
            }
            $this->logLn('Bummer, we did not receive an image');
            $this->log('What we received starts with: "');
            $this->logLn(\str_replace("\r", '', \str_replace("\n", '', \htmlentities(\substr($response, 0, 400)))) . '..."');
