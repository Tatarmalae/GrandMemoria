...
class Gd extends AbstractConverter
{
    ...
    {
        return ['alpha-quality', 'auto-filter', 'encoding', 'low-memory', 'metadata', 'method', 'near-lossless', 'preset', 'sharp-yuv', 'size-in-percentage'];
    }
    ...
    public function checkOperationality()
    {
        ...
        if (!\function_exists('imagewebp')) {
            throw new SystemRequirementsNotMetException('Gd has been compiled without webp support.');
        }
        if (!\function_exists('imagepalettetotruecolor')) {
            if (!self::functionsExist(['imagecreatetruecolor', 'imagealphablending', 'imagecolorallocatealpha', 'imagefilledrectangle', 'imagecopy', 'imagedestroy', 'imagesx', 'imagesy'])) {
                throw new SystemRequirementsNotMetException('Gd cannot convert palette color images to RGB. ' . 'Even though it would be possible to convert RGB images to webp with Gd, ' . 'we refuse to do it. A partial working converter causes more trouble than ' . 'a non-working. To make this converter work, you need the imagepalettetotruecolor() ' . 'function to be enabled on your system.');
            }
        }
    }
    /**
        ...
        switch ($mimeType) {
            case 'image/png':
                if (!\function_exists('imagecreatefrompng')) {
                    throw new SystemRequirementsNotMetException('Gd has been compiled without PNG support and can therefore not convert this PNG image.');
                }
                break;
            case 'image/jpeg':
                if (!\function_exists('imagecreatefromjpeg')) {
                    throw new SystemRequirementsNotMetException('Gd has been compiled without Jpeg support and can therefore not convert this jpeg image.');
                }
        }
    }
    /**
            ...
            if (\imagealphablending($dst, \false) !== \false) {
                ...
                if ($transparent !== \false) {
                    ...
                    if (\imagefilledrectangle($dst, 0, 0, \imagesx($image), \imagesy($image), $transparent) !== \false) {
                        ...
                        if (\imagealphablending($dst, \true) !== \false) {
    ...
    private function createImageResource()
    {
        ...
        switch ($mimeType) {
            case 'image/png':
                ...
                if ($image === \false) {
                    throw new ConversionFailedException('Gd failed when trying to load/create image (imagecreatefrompng() failed)');
                }
                return $image;
            case 'image/jpeg':
    ...
    protected function tryToMakeTrueColorIfNot(&$image)
    {
        ...
        if ($mustMakeTrueColor) {
            ...
            if ($success) {
                ...
                $this->logLn('FAILED converting color palette to true color. ');
            }
        }
        return $whatIsItNow;
    }
    ...
    protected function trySettingAlphaBlending($image)
    {
        if (\function_exists('imagealphablending')) {
            ...
            // changing anything...
            if (!\imagealphablending($image, \true)) {
    ...
    protected function errorHandlerWhileCreatingWebP($errno, $errstr, $errfile, $errline)
    {
        ...
        $this->errorMessageWhileCreating = $errstr . ' in ' . $errfile . ', line ' . $errline . ', PHP ' . \PHP_VERSION . ' (' . \PHP_OS . ')';
        //return false;
    }
    /**
     *
    ...
    protected function tryConverting($image)
    {
        ...
        if (!$success) {
            ...
            \ob_end_clean();
            \restore_error_handler();
            throw new ConversionFailedException('Failed creating image. Call to imagewebp() failed.', $this->errorMessageWhileCreating);
        }
        // The following hack solves an `imagewebp` bug
        // See https://stackoverflow.com/questions/30078090/imagewebp-php-creates-corrupted-webp-files
        if (\ob_get_length() % 2 == 1) {
            echo "\x00";
            $addedZeroPadding = \true;
        }
        $output = \ob_get_clean();
        \restore_error_handler();
        if ($output == '') {
            $this->destroyAndRemove($image);
            throw new ConversionFailedException('Gd failed: imagewebp() returned empty string');
        }
        // --------------------------------- (end of danger zone).
        if ($this->errorMessageWhileCreating != '') {
            switch ($this->errorNumberWhileCreating) {
                ...
                default:
                    $this->destroyAndRemove($image);
                    throw new ConversionFailedException('An error was produced during conversion', $this->errorMessageWhileCreating);
            }
        }
        if ($addedZeroPadding) {
        ...
        if (!$success) {
            $this->destroyAndRemove($image);
            throw new ConversionFailedException('Gd failed when trying to save the image. Check file permissions!');
        }
        /*
    ...
    protected function doActualConvert()
    {
        ...
        if ($isItTrueColorNow === \false) {
            ...
            throw new ConversionFailedException('Cannot convert image because it is a palette image and the palette image cannot ' . 'be converted to RGB (which is required). To convert to RGB, we would need ' . 'imagepalettetotruecolor(), which is not available on your system. ' . 'Our workaround does not have the neccasary functions for converting to RGB either.');
        }
        if (\is_null($isItTrueColorNow)) {
            $isWindows = \preg_match('/^win/i', \PHP_OS);
            $isMacDarwin = \preg_match('/^darwin/i', \PHP_OS);
            // actually no problem in PHP 7.4 and 8.0
            if ($isWindows || $isMacDarwin) {
                throw new ConversionFailedException('Cannot convert image because it appears to be a palette image and the palette image ' . 'cannot be converted to RGB, as you do not have imagepalettetotruecolor() enabled. ' . 'Converting palette on ' . ($isWindows ? 'Windows causes FATAL error' : 'Mac causes halt') . 'So we abort now');
            }
        }
        if ($this->getMimeTypeOfSource() == 'image/png') {
            if (\function_exists('version_compare')) {
                if (\version_compare($versionString, "2.1.1", "<=")) {
                    $this->logLn('BEWARE: Your version of Gd looses the alpha chanel when converting to webp.' . 'You should upgrade Gd, use another converter or stop converting PNGs. ' . 'See: https://github.com/rosell-dk/webp-convert/issues/238');
                } elseif (\version_compare($versionString, "2.2.4", "<=")) {
                    $this->logLn('BEWARE: Older versions of Gd looses the alpha chanel when converting to webp.' . 'We have not tested if transparency fails on your (rather old) version of Gd. ' . 'Please let us know. ' . 'See: https://github.com/rosell-dk/webp-convert/issues/238');
                }
            }
