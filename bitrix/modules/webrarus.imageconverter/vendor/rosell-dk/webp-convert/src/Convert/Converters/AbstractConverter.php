...
use Rarus\Isolated\WebPConvert\Convert\Converters\BaseTraits\AutoQualityTrait;
use Rarus\Isolated\WebPConvert\Convert\Converters\BaseTraits\DestinationPreparationTrait;
use Rarus\Isolated\WebPConvert\Convert\Converters\BaseTraits\LoggerTrait;
use Rarus\Isolated\WebPConvert\Convert\Converters\BaseTraits\OptionsTrait;
...
use Rarus\Isolated\WebPConvert\Loggers\BaseLogger;
/**
    ...
    private function doConvertImplementation()
    {
        ...
        if (!@\file_exists($destination)) {
            ...
            if (!isset($this->options['_suppress_success_message'])) {
                $this->ln();
                $this->log('Converted image in ' . \round((\microtime(\true) - $beginTime) * 1000) . ' ms');
                $sourceSize = @\filesize($source);
                if ($sourceSize !== \false) {
            ...
            */
            ...
            foreach ($e->getTrace() as $trace) {
                ...
                if (isset($trace['file']) && isset($trace['line'])) {
                    $this->logLn($trace['file'] . ':' . $trace['line']);
                }
            }
            throw $e;
        } catch (\Throwable $e) {
