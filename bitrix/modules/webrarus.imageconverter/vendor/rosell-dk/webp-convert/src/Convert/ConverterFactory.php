...
namespace Rarus\Isolated\WebPConvert\Convert;

use Rarus\Isolated\WebPConvert\Convert\Exceptions\ConversionFailed\InvalidInput\ConverterNotFoundException;
...
class ConverterFactory
{
    ...
    public static function converterIdToClassname($converterId)
    {
        ...
        $className = 'Rarus\\Isolated\\WebPConvert\\Convert\\Converters\\' . $classNameShort;
     ...
     */
    public static function makeConverterFromClassname($converterClassName, $source, $destination, $options = [], $logger = null)
    {
        if (!\is_callable([$converterClassName, 'convert'])) {
            throw new ConverterNotFoundException('There is no converter with class name:' . $converterClassName . ' (or it is not a converter)');
        }
        //$converter = new $converterClassName($source, $destination, $options, $logger);
        return \call_user_func([$converterClassName, 'createInstance'], $source, $destination, $options, $logger);
    }
    /**
