...
class Vips extends AbstractConverter
{
    ...
    {
        return ['auto-filter', 'size-in-percentage'];
    }
    /**
    ...
    public function checkOperationality()
    {
        ...
        if (!\function_exists('Rarus\\Isolated\\vips_image_new_from_file')) {
            throw new SystemRequirementsNotMetException('Vips extension seems to be installed, however something is not right: ' . 'the function "vips_image_new_from_file" is not available.');
        }
        if (!\function_exists('Rarus\\Isolated\\vips_call')) {
            throw new SystemRequirementsNotMetException('Vips extension seems to be installed, however something is not right: ' . 'the function "vips_call" is not available.');
        }
        if (!\function_exists('Rarus\\Isolated\\vips_error_buffer')) {
            throw new SystemRequirementsNotMetException('Vips extension seems to be installed, however something is not right: ' . 'the function "vips_error_buffer" is not available.');
        }
        vips_error_buffer();
        // clear error buffer
        ...
        if ($result === -1) {
            ...
            if (\strpos($message, 'VipsOperation: class "webpsave" not found') === 0) {
                throw new SystemRequirementsNotMetException('Vips has not been compiled with webp support.');
            }
        }
    }
    /**
    ...
    public function checkConvertability()
    {
        ...
        if (\function_exists('Rarus\\Isolated\\vips_version')) {
    ...
    private function createImageResource()
    {
        ...
        if (!\is_array($result)) {
            throw new ConversionFailedException('vips_image_new_from_file did not return an array, which we expected');
        }
        if (\count($result) != 1) {
            throw new ConversionFailedException('vips_image_new_from_file did not return an array of length 1 as we expected ' . '- length was: ' . \count($result));
        }
    ...
    private function createParamsForVipsWebPSave()
    {
        ...
        $options = ["Q" => $this->getCalculatedQuality(), 'lossless' => $this->options['encoding'] == 'lossless', 'strip' => $this->options['metadata'] == 'none'];
        ...
        if ($this->options['smart-subsample'] !== \false) {
            ...
            $this->logLn('*Note: the "smart-subsample" option is now deprecated. It turned out it corresponded to ' . 'the general option "sharp-yuv". You should use "sharp-yuv" instead.*');
        }
        ...
        if (!\is_null($this->options['preset']) && $this->options['preset'] != 'none') {
            ...
            $options['preset'] = \array_search($this->options['preset'], ['default', 'picture', 'photo', 'drawing', 'icon', 'text']);
        }
    ...
    private function webpsave($im, $options)
    {
        ...
        if ($result === -1) {
            ...
            if (\preg_match("#no property named .(.*).#", $message, $matches)) {
                ...
                if (\in_array($matches[1], ['lossless', 'alpha_q', 'near_lossless', 'smart_subsample', 'reduction_effort', 'preset'])) {
                    $nameOfPropertyNotFound = $matches[1];
                }
            }
