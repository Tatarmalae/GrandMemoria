...
namespace Rarus\Isolated\WebPConvert\Convert\Helpers;
...
class JpegQualityDetector
{
    ...
    private static function detectQualityOfJpgUsingImageMagick($filename)
    {
        if (ExecWithFallback::anyAvailable()) {
            ...
            try {
                ExecWithFallback::exec("identify -format '%Q' " . \escapeshellarg($filename) . " 2>&1", $output, $returnCode);
    ...
    private static function detectQualityOfJpgUsingGraphicsMagick($filename)
    {
        if (ExecWithFallback::anyAvailable()) {
            // Try GraphicsMagick
            try {
                ExecWithFallback::exec("gm identify -format '%Q' " . \escapeshellarg($filename) . " 2>&1", $output, $returnCode);
                if (\intval($returnCode) == 0 && \is_array($output) && \count($output) == 1) {
