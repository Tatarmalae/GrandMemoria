...
use Rarus\Isolated\WebPConvert\Convert\Exceptions\ConversionFailed\FileSystemProblems\CreateDestinationFolderException;
/**
...
trait DestinationPreparationTrait
{
    ...
    private function createWritableDestinationFolder()
    {
        ...
        if (!\file_exists($folder)) {
            ...
            if (!\mkdir($folder, 0777, \true)) {
                throw new CreateDestinationFolderException('Failed creating folder. Check the permissions!', 'Failed creating folder: ' . $folder . '. Check permissions!');
            }
        }
    }
    /**
    ...
    private function checkDestinationWritable()
    {
        ...
        throw new CreateDestinationFileException('Cannot create file: ' . \basename($destination) . ' in dir:' . \dirname($destination));
    }
    /**
    ...
    private function removeExistingDestinationIfExists()
    {
        ...
        if (\file_exists($destination)) {
            ...
            if (!\unlink($destination)) {
                throw new CreateDestinationFileException('Existing file cannot be removed: ' . \basename($destination));
            }
        }
    }
}
