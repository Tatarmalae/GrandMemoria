...
use Rarus\Isolated\LocateBinaries\LocateBinaries;
use Rarus\Isolated\WebPConvert\Convert\Converters\AbstractConverter;
use Rarus\Isolated\WebPConvert\Convert\Converters\BaseTraits\WarningLoggerTrait;
use Rarus\Isolated\WebPConvert\Convert\Converters\ConverterTraits\EncodingAutoTrait;
use Rarus\Isolated\WebPConvert\Convert\Converters\ConverterTraits\ExecTrait;
use Rarus\Isolated\WebPConvert\Convert\Exceptions\ConversionFailed\ConverterNotOperational\SystemRequirementsNotMetException;
...
class Cwebp extends AbstractConverter
{
    ...
    private static $suppliedBinariesInfo = ['WINNT' => [
    ...
    ], 'Darwin' => [['cwebp-110-mac-10_15', 'bfce742da09b959f9f2929ba808fed9ade25c8025530434b6a47d217a6d2ceb5', '1.1.0']], 'SunOS' => [
        ...
        ['cwebp-103-linux-x86-64-static', 'ab96f01b49336da8b976c498528080ff614112d5985da69943b48e0cb1c5228a', '1.0.3'],
        ...
        foreach (self::$suppliedBinariesInfo as $os => $arr) {
            foreach ($arr as $i => list($filename, $expectedHash)) {
                ...
                if ($expectedHash != $actualHash) {
                    throw new \Exception('Hash for ' . $filename . ' is incorrect! ' . 'Checksum is: ' . $actualHash . ', ' . ', but expected: ' . $expectedHash . '. Did you transfer with FTP, but not in binary mode? ');
                }
            }
        }
    }
    public function checkOperationality()
    {
        ...
        if (!$options['try-supplied-binary-for-os'] && !$options['try-common-system-paths'] && !$options['try-cwebp'] && !$options['try-discovering-cwebp']) {
            throw new ConverterNotOperationalException('Configured to neither try pure cwebp command, ' . 'nor look for cweb binaries in common system locations and ' . 'nor to use one of the supplied precompiled binaries. ' . 'But these are the only ways this converter can convert images. No conversion can be made!');
        }
    }
    ...
    private function createCommandLineOptions($version)
    {
        ...
        if (isset($matches[0])) {
            ...
            $this->logLn('Could not extract version number from the following version string: ' . $version, 'bold');
        }
        ...
        if ($options['near-lossless'] !== 100) {
            if ($versionNum < 0.5) {
                ...
                if ($options['encoding'] == 'lossless') {
                    ...
                    $this->logLn('The near-lossless option ignored for lossy');
                }
            }
        }
        // Autofilter
        if ($options['auto-filter'] === \true) {
            $cmdOptions[] = '-af';
        }
        // SharpYUV
        if ($options['sharp-yuv'] === \true) {
            if ($versionNum >= 0.6) {
                // #284
    ...
    private function checkHashForSuppliedBinary($binaryFile, $hash)
    {
        ...
        if (\function_exists('hash_file')) {
            $this->logLn('Checking checksum for supplied binary: ' . $binaryFile);
            $startHashCheckTime = self::startTimer();
            $binaryHash = \hash_file('sha256', $binaryFile);
            if ($binaryHash != $hash) {
                $this->logLn('Binary checksum of supplied binary is invalid! ' . 'Did you transfer with FTP, but not in binary mode? ' . 'File:' . $binaryFile . '. ' . 'Expected checksum: ' . $hash . '. ' . 'Actual checksum:' . $binaryHash . '.', 'bold');
                return \false;
            }
    ...
    private function detectVersion($binary)
    {
        ...
        if ($returnCode == 0) {
            ...
            if ($returnCode == 127) {
                ...
                if ($returnCode == 126) {
                    $this->logLn('*Exec failed*. ' . 'Permission denied (' . $this->who() . ' does not have permission to execute that binary)');
                } else {
        ...
        return '';
        // Will not happen. Just so phpstan doesn't complain
    }
    /**
    ...
    private function logDiscoverAction($optionName, $description)
    {
        if ($this->options[$optionName]) {
            $this->logLn('Discovering binaries ' . $description . ' ' . '(to skip this step, disable the "' . $optionName . '" option)');
        } else {
            $this->logLn('Skipped discovering binaries ' . $description . ' ' . '(enable "' . $optionName . '" if you do not want to skip that step)');
        }
    }
    ...
    {
        $this->logLn('Looking for cwebp binaries.');
        ...
        if (!empty(\getenv('WEBPCONVERT_CWEBP_PATH'))) {
            $this->logLn('WEBPCONVERT_CWEBP_PATH environment variable was set, so using that path and ignoring any other');
            return [[\getenv('WEBPCONVERT_CWEBP_PATH')], [[], []]];
        }
        if ($this->options['try-cwebp']) {
            $startTime = self::startTimer();
            $this->logLn('Discovering if a plain cwebp call works (to skip this step, disable the "try-cwebp" option)');
            $result = $this->detectVersion('cwebp');
            if (\gettype($result) == 'string') {
                $this->logLn('We could get the version, so yes, a plain cwebp call works ' . '(spent ' . self::readTimer($startTime) . ' ms)');
                $binaries[] = 'cwebp';
            } else {
                $this->logLn('Nope a plain cwebp call does not work' . self::getTimeStr($startTime));
            }
        } else {
            $this->logLn('Skipped discovering if a plain cwebp call works' . ' (enable the "try-cwebp" option if you do not want to skip that step)');
        }
    ...
    protected function doActualConvert()
    {
        ...
        if (\count($allBinaries) == 0) {
            ...
            $discoverOptions = ['try-supplied-binary-for-os', 'try-common-system-paths', 'try-cwebp', 'try-discovering-cwebp'];
            ...
            if (\count($disabledDiscoverOptions) == 0) {
                throw new SystemRequirementsNotMetException('No cwebp binaries found.');
            } else {
                throw new SystemRequirementsNotMetException('No cwebp binaries found. Try enabling the "' . \implode('" option or the "', $disabledDiscoverOptions) . '" option.');
            }
        }
        $detectedVersions = [];
        if (\count($foundBinaries) > 0) {
            $this->logLn('Detecting versions of the cwebp binaries found' . (\count($suppliedBinaries) > 0 ? ' (except supplied binaries)' : '.'));
        ...
        $this->logLn('Binaries ordered by version number.');
        foreach ($binaryVersions as $binary => $version) {
            $this->logLn('- ' . $binary . ': (version: ' . $version . ')');
        }
        // Execute!
        $this->logLn('Starting conversion, using the first of these. If that should fail, ' . 'the next will be tried and so on.');
        ...
        foreach ($binaryVersions as $binary => $version) {
            ...
            if ($this->tryCwebpBinary($binary, $version, $useNice)) {
                ...
                if (isset($suppliedBinariesFilename[$binary])) {
                    $this->logLn('Note: You can prevent trying this precompiled binary, by setting the ' . '"skip-these-precompiled-binaries" option to "' . $suppliedBinariesFilename[$binary] . '"');
                }
            }
        }
        ...
        if ($success) {
            ...
            if ($fileStatistics !== \false) {
                ...
                $permissions = $fileStatistics['mode'] & 0666;
