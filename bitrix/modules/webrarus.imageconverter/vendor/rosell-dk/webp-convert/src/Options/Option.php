...
class Option
{
    ...
    protected function checkType($expectedType)
    {
        if (\gettype($this->getValue()) != $expectedType) {
            throw new InvalidOptionTypeException('The "' . $this->id . '" option must be a ' . $expectedType . ' (you provided a ' . \gettype($this->getValue()) . ')');
        }
    }
    ...
    {
        return \print_r($this->getValue(), \true);
    }
    /**
    ...
    {
        $obj = ['id' => $this->id, 'schema' => $this->getSchema(), 'ui' => $this->ui];
        if ($this->deprecated) {
            $obj['deprecated'] = \true;
        }
        return $obj;
