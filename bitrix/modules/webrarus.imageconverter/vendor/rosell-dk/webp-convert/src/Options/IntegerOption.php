...
class IntegerOption extends Option
{
    ...
    protected function checkMin()
    {
        if (!\is_null($this->minValue) && $this->getValue() < $this->minValue) {
            throw new InvalidOptionValueException('"' . $this->id . '" option must be set to minimum ' . $this->minValue . '. ' . 'It was however set to: ' . $this->getValue());
        }
    }
    protected function checkMax()
    {
        if (!\is_null($this->maxValue) && $this->getValue() > $this->maxValue) {
            throw new InvalidOptionValueException('"' . $this->id . '" option must be set to max ' . $this->maxValue . '. ' . 'It was however set to: ' . $this->getValue());
        }
    }
