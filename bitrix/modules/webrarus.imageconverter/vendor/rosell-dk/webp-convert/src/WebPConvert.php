...
namespace Rarus\Isolated\WebPConvert;
...
use Rarus\Isolated\WebPConvert\Serve\ServeConvertedWebP;
use Rarus\Isolated\WebPConvert\Serve\ServeConvertedWebPWithErrorHandling;
/**
...
class WebPConvert
{
     ...
     * @return void
     */
    public static function serveConverted($source, $destination, $options = [], $serveLogger = null, $convertLogger = null)
    {
     ...
     * @since 2.8.0
     */
    public static function getConverterOptionDefinitions($filterOutOptionsWithoutUI = \true)
    {
        ...
        \array_unshift($result['general'], OptionFactory::createOption('converter', 'string', ['title' => 'Converter', 'description' => 'Conversion method. ' . "Cwebp and vips are best. " . 'the *magick are nearly as good, but only recent versions supports near-lossless. ' . 'gd is poor, as it does not support any webp options. ' . 'For full discussion, check the guide', 'default' => 'stack', 'enum' => $converterIds, 'ui' => ['component' => 'select', 'links' => [['Guide', 'https://github.com/rosell-dk/webp-convert/blob/master/docs/v1.3/converting/converters.md']]]])->getDefinition());
        $supportedBy = [];
        $uniqueOptions = [];
