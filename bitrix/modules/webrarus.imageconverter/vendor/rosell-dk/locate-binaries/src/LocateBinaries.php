...
namespace Rarus\Isolated\LocateBinaries;

use Rarus\Isolated\FileUtil\FileExists;
...
class LocateBinaries
{
    ...
    public static function locateInCommonSystemPaths($binary)
    {
        ...
        if (\stripos(\PHP_OS, 'WIN') === 0) {
            $commonSystemPaths = ['C:\\Windows\\System32'];
            $binary .= '.exe';
        } else {
            $commonSystemPaths = ['/usr/bin', '/usr/local/bin', '/usr/gnu/bin', '/usr/syno/bin', '/bin'];
        }
