...
use Rarus\Isolated\Psr\Log\AbstractLogger as PsrAbstractLogger;
use Rarus\Isolated\Psr\Log\InvalidArgumentException;
use Rarus\Isolated\Apix\Log\LogEntry;
use Rarus\Isolated\Apix\Log\LogFormatter;
/**
 * Abstratc class.
 *
...
abstract class AbstractLogger extends PsrAbstractLogger
{
     ...
     * @var array
     */
    protected static $levels = array('emergency', 'alert', 'critical', 'error', 'warning', 'notice', 'info', 'debug');
    /**
    ...
    public static function getLevelCode($level_name)
    {
        ...
        if (\false === $level_code) {
            throw new InvalidArgumentException(\sprintf('Invalid log level "%s"', $level_name));
        }
        return $level_code;
    }
    /**
     ...
     * @return self
     */
    public function setMinLevel($name, $cascading = \true)
    {
