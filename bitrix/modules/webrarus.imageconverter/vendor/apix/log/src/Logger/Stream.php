...
class Stream extends AbstractLogger implements LoggerInterface
{
    ...
    public function __construct($stream = 'php://stdout', $mode = 'a')
    {
        ...
        if (!\is_resource($stream)) {
            throw new InvalidArgumentException(\sprintf('The stream "%s" cannot be created or opened', $stream));
        }
        $this->stream = $stream;
    }
    /**
    ...
    public function write(LogEntry $log)
    {
        if (!\is_resource($this->stream)) {
            throw new \LogicException('The stream resource has been __destruct() too early');
        }
