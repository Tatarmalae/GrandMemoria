 ...
 */
namespace Rarus\Isolated\Apix\Log;

use Rarus\Isolated\Apix\Log\Logger\AbstractLogger;
...
class Logger extends AbstractLogger
{
    ...
    public function __construct(array $loggers = array())
    {
        foreach ($loggers as $key => $logger) {
            if ($logger instanceof Logger\LoggerInterface) {
                ...
                throw new InvalidArgumentException(\sprintf('"%s" must interface "%s".', \get_class($logger), __NAMESPACE__ . '\\Logger\\LoggerInterface'));
            }
        }
        $this->sortBuckets();
    }
    ...
    public function process(LogEntry $log)
    {
        ...
        if (\false !== $i) {
            while (isset($this->buckets[$i]) && $this->buckets[$i]->process($log)) {
                ++$i;
            }
            return \true;
        }
    ...
    public static function getPsrLevelName($level_name)
    {
        ...
        if (!\defined($logLevel)) {
            throw new InvalidArgumentException(\sprintf('Invalid PSR-3 log level "%s"', $level_name));
        }
        return $level_name;
    }
    /**
